...
// Allowed fulfillment states (must match DB CHECK constraint)
const FULFILLMENT_ALLOWED = [
  "unfulfilled",
  "processing",
  "in_transit",
  "shipped",
...
const TABS = [
  { key: "unshipped", label: "Unshipped" },   // unfulfilled + processing
  { key: "in_transit", label: "In Transit" }, // in_transit
  { key: "shipped", label: "Shipped" },       // shipped + delivered
  { key: "all", label: "All" },
];
...
export default function AdminOrders() {
  ...
  async function handleFulfillmentChange(row, nextStatus) {
    if (!FULFILLMENT_ALLOWED.includes(nextStatus)) {
      toast.error(`Invalid status: ${nextStatus}`);
      return;
    }
    try {
                              ...
                              <div key={oi.id} className="ao-item">
                                ...
                                <div className="ao-item-meta">
                                  ...
                                  <div className="ao-item-sub">
                                    {qty} × {php(unit)} = <strong>{php(line)}</strong>
                                  </div>
                                </div>
                              </div>
                            );
                          ...
                          {(row.cadavers ?? []).map((cd) => (
                            <div key={cd.id} className="ao-cadaver-card">
                              ...
                              {cd.death_certificate_url && (
                                <div>
                                  <a href={cd.death_certificate_url} target="_blank" rel="noreferrer">
                                    Death certificate
                                  </a>
                                </div>
                              )}
